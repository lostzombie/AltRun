...
begin
  ...
  begin
    ...
    begin
      ...
      Inc(i);
      if i <= Length(TmpStr) - 1 then
        TmpStr2[j] := TmpStr2[j] + ' ' + TmpStr[i];
    end;
    Inc(i);
    Inc(j);
  end;
  ...
  begin
    ...
    begin
      ...
      Inc(i);
      if i <= Length(TmpStr2) - 1 then
        TmpStr3[j] := TmpStr3[j] + ' ' + TmpStr2[i];
    end;
    if Length(TmpStr3[j]) = 0 then
      Dec(j);
    Inc(i);
    Inc(j);
  end;
  ...
  begin
    if CharCount(sq, TmpStr3[i]) = 2 then
      if (TmpStr3[i][1] = sq) and (TmpStr3[i][Length(TmpStr3[i])] = sq) then
        TmpStr3[i] := Copy(TmpStr3[i], 2, Length(TmpStr3[i]) - 2);
    if Pos(sq, TmpStr3[i]) > 0 then
      TmpStr3[i] := StringReplace(TmpStr3[i], sq, '', [rfReplaceAll]);
    if CharCount(dq, TmpStr3[i]) = 2 then
      if (TmpStr3[i][1] = dq) and (TmpStr3[i][Length(TmpStr3[i])] = dq) then
        TmpStr3[i] := Copy(TmpStr3[i], 2, Length(TmpStr3[i]) - 2);
  end;
  i := 0;
  while ((TmpStr3[i][1] <> hyp) and (i < Length(TmpStr3) - 1)) do
    Inc(i);
  ...
  begin
    ...
    for j := 0 to i - 1 do
      if ParamStr[0] = '' then
        ParamStr[0] := ParamStr[0] + TmpStr3[j]
      else
        ParamStr[0] := ParamStr[0] + ' ' + TmpStr3[j];
    for j := i to Length(TmpStr3) - 1 do
      ParamStr[j - i + 1] := TmpStr3[j];
  end
  else
    ParamStr := TmpStr3;
...
begin
  Result := False;
  if Mode = 'File' then
    FileCfg := TFileStream.Create(Name, 0);
  if Mode = 'Res' then
    ResCfg := TResourceStream.Create(HInstance, Name, PChar(10));
  i := 0;
  ReadOK := False;
  repeat
    Tmp := '';
    if Mode = 'File' then
      ReadOK := ReadStreamLine(FileCfg, Tmp);
    if Mode = 'Res' then
      ReadOK := ReadStreamLine(ResCfg, Tmp);
    if ReadOK then
    begin
...
begin
  ...
  begin
    if hp(ParamCount, ParamStr, 'r', 'run') or hp(ParamCount, ParamStr, 'rb', 'right-button') or hp(ParamCount, ParamStr, 'c', ctrl) or
      hp(ParamCount, ParamStr, 'a', alt) or hp(ParamCount, ParamStr, 's', shift) or hp(ParamCount, ParamStr, 'ca', ctrl + hyp + alt) or
      hp(ParamCount, ParamStr, 'cs', ctrl + hyp + shift) or hp(ParamCount, ParamStr, 'as', alt + hyp + shift) or hp(ParamCount, ParamStr, 'cas', ctrl + hyp + alt + hyp + shift) then
      Result := True
    else
    begin
      if ParamCount > 1 then
      begin
        if pos(space, ParamStr[1]) > 0 then
          o := dq + ParamStr[1] + dq
        else
          o := ParamStr[1];
        for i := 2 to ParamCount - 1 do
          if pos(space, ParamStr[i]) > 0 then
            o := o + ' ' + dq + ParamStr[i] + dq
          else
            o := o + ' ' + ParamStr[i];
      end;
      ...
      begin
        if GetParamsFrom('File', CfgName, ParamStr[0], o, ParamCount, ParamStr) then
          Result := True;
      end;
    end;
  end
  ...
  begin
    ...
    if (Pos(dots, CfgName) = 0) then
      CfgName := RelToAbs(CfgName, GetCurrentDir);
    if not Exists(CfgName) then
    begin
      CfgName := gp(ParamCount, ParamStr, 'g', 'cfg');
      if (Pos(dots, CfgName) = 0) then
        CfgName := RelToAbs(CfgName, ExtractFilePath(ParamStr[0]));
    end;
    if Exists(CfgName) then
    begin
      if GetParamsFrom('File', CfgName, ParamStr[0], o, ParamCount, ParamStr) then
  ...
  if not Result then
    if GetParamsFrom('Res', 'CONFIG', ParamStr[0], o, ParamCount, ParamStr) then
      Result := True;
end;
...
begin
  ...
  begin
    if ((AIndex + 1) < ParamCount) then
      Result := ParamStr[AIndex + 1];
  end;
end;
...
begin
  Result := False;
  if FindParamIndex(ParamCount, ParamStr, S1, B) <> 0 or FindParamIndex(ParamCount, ParamStr, S2, B) then
    Result := True;
end;
...
begin
  ...
  if (I = 0) then
    I := FindParamIndex(ParamCount, ParamStr, S, B);
  if I <> 0 then
    Result := GetParamAtIndex(ParamCount, ParamStr, I, B);
  if Result = '""' then
    Result := ''
  else
  if Result <> '' then
    if (((Result[1] = dq) and (Result[Length(Result)] = dq)) and (CharCount(dq, Result) = 2)) then
